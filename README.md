...
figure on a random live sample. And for the ~99% that ship no spec,
[`demos/cold_golden/`](demos/cold_golden/) maps how much is checkable *without* author
input: ~half the marketplace ships runnable code, ~30% of the catalog is deterministically
checkable once you count the request-build skeleton of network skills, and a thin ~1.7%
of code-bearing skills are cold-confirmable from the docs alone — a map of the gap, never
an accusation.
...
- **Sandboxing untrusted code is the host's job — and vetting is fail-closed about
  it.** Skill/memory vetting runs candidate code only when you opt in
  (`permit_execution=True`); the `self_spec` demo refuses to run anything without an
  explicit `--allow-host-exec` vow and can isolate the worker with `--sandbox
  docker:<image>`. Run it where that's acceptable.
...
axis — are implemented and tested (the full suite passes; `pytest` in `tests/`). The verdict re-derivation path itself is stdlib-only
