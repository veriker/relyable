...
# Untrusted entrypoint code must not see the checker's environment (keys,
# tokens): allowlist, never inherit. Inline (stdlib-only grader; no relyable
# import per the auditor-independence contract).
_ENV_ALLOWED = ("PATH", "HOME", "LANG", "LC_ALL", "LC_CTYPE", "TMPDIR", "TMP",
                "TEMP", "PYTHONIOENCODING", "SYSTEMROOT", "COMSPEC")


def _scrubbed_env() -> dict:
    env = {{k: v for k in _ENV_ALLOWED if (v := os.environ.get(k)) is not None}}
